...
namespace mustache {
    ...
    public:
        ...
        MUSTACHE_INLINE T* allocate(size_t count) noexcept;

        MUSTACHE_INLINE void deallocate(T* ptr, size_t /*count*/) noexcept;
    ...
    template<typename T>
    T* Allocator<T>::allocate(size_t count) noexcept {
        auto ptr = manager_->allocate(sizeof(T) * count, alignof(T));
        return static_cast<T*>(ptr);
    }
    template<typename T>
    void Allocator<T>::deallocate(T* ptr, size_t /*count*/) noexcept {
